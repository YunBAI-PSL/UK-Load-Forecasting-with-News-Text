...
def createXYtables4training(configuration,lags,NIE=False):
    try:
        ...
        # filter the outliers, replace with the mean of last week
        window=48*7
        roll = df.rolling(window)
        mps_up, mps_low = roll.mean()+3*roll.std(), roll.mean()-3*roll.std()       
        df[window:].loc[~df['Load'][window:].between(mps_low.Load[window:], mps_up.Load[window:]), 'Load'] = np.NaN
        del mps_up, mps_low
        ...
        # add temperatures
        temperature = get_temperature(lags)
